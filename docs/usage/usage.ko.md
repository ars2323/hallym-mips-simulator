...
| **Registers** | 32개 레지스터와 PC·HI·LO. 쓰임새별로 묶여 있습니다(Arguments, Temporaries, Saved …). 방금 바뀐 레지스터는 **노란 줄**(넓은 창에서는 줄 끝에 **Changed** 표). 상태 표시줄의 노란 칸 *방금 바뀜* 에도 그 이름이 나옵니다. **Hex** · **Dec** · **Bin** 은 같은 값의 16진·10진·2진 |
...
| **상태 표시줄** | 맨 아래. 지금 상태(한 줄 실행했습니다, 브레이크포인트에서 멈췄습니다 …), 단계 수, 방금 바뀐 레지스터(노란 칸 *방금 바뀜: …*). Ctrl+S 를 누른 뒤에는 저장했는지도(*저장됨*, *예제라서 저장하지 않습니다* …) |
