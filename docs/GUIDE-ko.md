...
- 다시 보려면 **Help > Tutorial**을 누릅니다. **편집기가 비어 있을 때만** 예제를 엽니다. 편집 중이거나 파일을 열어 둔 상태라면 예제를 열지 않고 그 화면 그대로 진행하며, 저장 여부를 묻지 않고 작업 중인 파일도 건드리지 않습니다.
- 그 경우 1단계 카드에 **[예제로 보기]** 버튼이 있습니다. 누르면 예제 프로그램으로 바꿔 18단계 전체를 처음부터 보여 줍니다. 저장하지 않은 내용이 있으면 이때 저장할지 물어보고, 취소하면 아무것도 바뀌지 않습니다.
