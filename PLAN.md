...
| 인스펙터 높이 | 지금은 내용 6줄 고정. 5단계에서 명령어 필드 분해가 들어가면 "선택 대상에 따라 내용에 맞춤, 최대 N줄"로 바꾼다 | 인스펙터가 쓰는 줄만큼 레지스터 목록이 줄어든다 |
...
| 검증 절차 | 단계별 사람 확인은 **Linux**에서. Windows 확인은 **7단계 진입 전**과 **8단계**에서 한 번에 | push마다 CI가 Windows 빌드·단위 테스트·zip 패키징을 돌리므로 컴파일 문제는 계속 걸러진다 |
...
- 타입 배지: **R / I / J**, 보조프로세서 명령은 **FR / FI**(P&H green card 기준), CP0 명령은 **CP0**으로 표기
  - **판정은 기계어 워드만으로**: opcode 0 → R, 2·3 → J, 0x10 → CP0, 0x11 → fmt 필드로 FR/FI, 나머지 → I
...
- **Assemble**: 저장 → 원본 "Reinitialize and Load File" 경로 그대로 호출 → **성공 시 Text 탭으로 전환, 실패 시 Editor에 머물고 에러 줄 표시**
...
6. **사람 체크포인트**: 단계마다 실제 모니터(Linux)에서 확인. Windows zip 확인은 7단계 진입 전과 8단계에서 몰아서 한다 (결정 표 "검증 절차")

## 단계

각 단계 끝에 사람 확인(Linux) 후 `stage-N` 태그. Windows 확인은 7단계 진입 전과 8단계.
