/* QtSpim-Edu: the inspector's text for one instruction (PLAN R3).
   ...
   Branches and jumps get their destination and, in SPIM's default branch
   mode, the note agreed at the stage-4 checkpoint.

   Pure text layout, QtCore only; meant for a fixed-pitch font.  The table
   part never exceeds kInstructionTextColumns characters, so it fits the
   inspector without wrapping; the destination and note lines may be longer
   and are expected to be word-wrapped by the widget.
*/
