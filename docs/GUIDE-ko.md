...
- 다시 보려면 **Help > Tutorial**을 누릅니다. **편집기가 비어 있을 때만** 예제를 엽니다. 편집 중이거나 파일을 열어 둔 상태라면 예제를 열지 않고 그 화면 그대로 진행하며, 저장 여부를 묻지 않고 작업 중인 파일도 건드리지 않습니다. 예제로 보고 싶으면 편집기를 비우고 다시 실행하세요.
