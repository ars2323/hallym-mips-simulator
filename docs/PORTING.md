...
- `yy_scan_bytes` 는 flex 2.6 의 공개 함수이고 win_flex 도 flex 2.6 계열이다. 그래서 Windows 에서도
  같은 코드가 돌 것으로 **예상한다.** 아직 시도하지 않았다.
...
- **브레이크포인트 표시**: `text-breakpoint` 골든에는 업스트림이 고정 오프셋으로 자른 줄이 들어 있다
  (`README.qt.md`). 이 케이스는 브레이크포인트 설정(쓰기)이 애드온에 아직 없어서 건너뛴다.
...
| 벌 | 개수 | 증명하는 것 |
|---|---|---|
| Qt 환경 골든 | 29 (28 통과, 1 건너뜀) | 코어가 Qt판과 동일하다 |
...
include 하지 않는다. 그러니 MSVC 에서는 충돌할 것이 없고 msvc 블록에 짝이 필요 없다.
...
`signal(SIGALRM)`·`setitimer` 를 프로세스 전체에 건다. `fatal_error()` 는 반환하면 안 되는 함수라서
지금은 `abort()` 한다. `run_program()` 은 동기 호출이다.
그래서 다음 단계에서 코어를 별도 프로세스로 뺀다. `src/core/` 의 모듈은 전부 순수·동기이며
모듈 수준 상태가 없다. 상태는 `native/index.ts` 경계에만 있다.
