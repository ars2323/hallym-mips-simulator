...
("Speed: Instant") → 간격 좁히기 → 맨 마지막에 프로그램 이름(로고는 남음). 1024·910 에서도 "Hallym MIPS" 가 남고,
최소 창(760)에서만 이름이 빠진다. 버튼의 이름(Assemble·Run·Step·Reset)은 끝까지 남는다. 속도에는 "Run speed" 라는 이름을
