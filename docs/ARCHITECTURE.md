...
| 119 | **BB — Text 패널이 세로로 떨리던 이유는 스크롤이 아니라 전면 다시 그리기였다**. 불변식은 "한 동작에 세로 위치는 하나만 그려진다"이고, 계측은 `scrollContentsBy()`에서 **그려진 세로 위치의 나열**과 **패널의 몇 %가 다시 그려졌는지**를 스텝마다 본다(`--vscroll-report`). 재현 결과 스텝당 세로 이동은 **0회**인데 Text·Registers가 **매 스텝 100%** 다시 그려지고 있었다. 원인 셋: (가) `eduScrollRowOnly()`가 `EduKeepHorizontalScroll`로 viewport의 갱신을 껐다 켜는데, `setUpdatesEnabled(true)`는 위젯 전체를 `update()`한다 — 스크롤이 필요 없어도 매 스텝 전면 갱신(V 수정의 부작용). 이제 `eduScrollVerticallyTo()`가 **세로 스크롤바만** 움직여 가로를 건드리지 않으므로 가드 자체가 필요 없다. (나) `dataChanged`가 **한 셀보다 넓으면 Qt는 viewport 전체를 update**한다(`qabstractitemview.cpp`의 `dataChanged`: `topLeft == bottomRight`만 좁은 경로). 행 전체를 한 신호로 알리던 것을 **셀 단위**로 바꿨다(`EduTextModel::markRowChanged`, `EduRegisterModel::refresh`). (다) 레지스터 패널이 매 스텝 `setFont`·`setPalette`·`ResizeToContents` 재계산을 하고 있었다 — 앞의 둘은 값이 같으면 건너뛰고, 헤더는 `Interactive`로 바꿔 폭을 `fitColumns()`가 글꼴·진법이 바뀔 때만 계산한다 | 스텝마다 세 패널이 통째로 다시 그려졌다 | 수정 후: 가운데에서 스텝하면 Text가 **9%**(두 행)만 다시 그려지고, 스크롤이 필요한 스텝은 세로 위치를 **하나만** 그린다. 레지스터는 바뀐 행들의 **바운딩 박스**가 넓어 퍼센트가 크게 나올 수 있는데(흩어진 행 3개면 그 사이가 다 포함된다) 그려지는 내용은 같으므로 깜빡임이 아니다 — 검사는 Text에만 퍼센트 기준을 건다. 검사: `check-editor.sh` 20절 | 1.2.3 · `edu/edu_view_scroll.*`, `edu_text_model.cpp`, `edu_register_model.cpp`, `edu_register_view.cpp` |
| 120 | **HH — Reset은 프로그램을 버리지 않고 다시 올린다**. 증상: 아무것도 고치지 않았는데 Reinitialize 뒤 Text 위에 **"Simulator was reinitialized — save (Ctrl+S) to assemble this file"**가 뜬다. 판정은 `edu_editor_glue.cpp`의 `eduUpdateStaleBanner()` — `eduSyncedPath.isEmpty() && eduEverAssembled` 가지이고, `eduForgetLoadedLabels()`가 초기화 때 `eduSyncedPath`를 비우기 때문이다. **mtime이 원인이라는 가설은 틀렸다**: 파일 비교는 처음부터 내용 해시다(`eduEditorDigest()`, MD5). 고친 방식은 문구가 아니라 동작이다 — 메뉴의 Reinitialize가 `eduReinitialize()`로 가고, 에디터가 저장된 파일을 들고 있으면 **비운 뒤 그 파일을 다시 어셈블**한다(X의 사이클을 저장 단계만 빼고 그대로 쓴다: 브레이크포인트·커서·가로 위치 유지, Messages 한 줄). 에디터가 비어 있거나 저장하지 않은 수정이 있으면 비우기만 한다 | 초기화가 프로그램을 통째로 버렸다 | Ctrl+S가 이미 "초기화 후 어셈블"인데 Reinitialize만 버리는 것은 앞뒤가 맞지 않았다. 검사: `check-editor.sh` 21절 — Reset 열 번에 배너 0회·Messages 10줄, Reset 뒤 바로 F5로 "Hello World", 브레이크포인트 유지, 한 글자 고치면 배너 복귀 | 1.2.3 · `edu_editor_glue.cpp`, `spimview.cpp` |
| 121 | **Text 위의 띠는 사실일 때만 뜬다**. 조건은 위에서 아래로 처음 맞는 것 하나 (`eduUpdateStaleBanner()`): <br>① 에디터가 비어 있고 이름도 없다 → **뜨지 않는다**(시작 화면이 대신 보인다) <br>② 시뮬레이터에 이 에디터의 것이 없고 **저장하지 않은 수정이 있다** → "Unsaved changes — save (Ctrl+S) to assemble this file" <br>③ 시뮬레이터에 이 에디터의 것이 없고 전에 어셈블한 적이 있다 → "Simulator was reinitialized — …"(HH 이후로는 메뉴 Reset으로는 나지 않는다) <br>④ 시뮬레이터에 이 에디터의 것이 없고 어셈블한 적도 없다 → "Not assembled yet — …" <br>⑤ 시뮬레이터가 **다른 파일**의 것을 들고 있다 → "Text shows a different program — …" <br>⑥ 에디터의 내용 해시가 어셈블된 것과 다르다 → "Source changed — …" | ②가 없어 ③이 대신 떴다 | ②를 넣은 이유: 초기화가 학생 자신의 동작이고 저장하지 않은 글이 있으면, 그때는 "무엇을 해야 하는지"를 정확히 말해야 한다 | 1.2.3 · `edu_editor_glue.cpp` |
| 122 | **GG — 대화상자에서 고른 크기는 남고, 키로 키운 것은 돌아간다**: 1.2.2는 Settings의 글꼴 종류·색은 유지하면서 **크기만** 매 실행 기본값으로 덮어써, 대화상자에서 고른 값이 조용히 안 먹는 것처럼 보였다. 이제 크기가 두 값이다 — **기준**(`st_panelPointSize`, 설정 파일의 `Panels/TextSize`, Settings의 "All panels text size"가 정한다)과 **오프셋**(`EduPanelZoom::offset_`, Ctrl+±·Ctrl+휠, 저장하지 않음). 화면 크기 = 기준 + 오프셋이고 Ctrl+0은 오프셋을 0으로 되돌린다. 기준을 바꿔도 진행 중인 확대는 유지된다. 덤으로 두 가지를 바로잡았다: `eduRefreshTextPanel()`이 매 갱신마다 Settings 글꼴의 **크기까지** 패널에 다시 씌워 확대를 되돌리고 있었고, 에디터가 Text 패널의 확대를 따라가 자기 Ctrl+±와 싸우고 있었다 — 이제 에디터는 글꼴 종류만 Settings에서 받고 크기는 자기 기준·오프셋을 쓴다 | 크기가 한 값이었고 매 실행 기본값으로 돌아갔다 | 학생이 이해할 규칙: **대화상자에서 고른 것은 남고, 키로 키운 것은 다음에 켜면 돌아온다.** 검사: `check-editor.sh` 22절, `--panel-size` | 1.2.3 · `edu/edu_panel_zoom.*`, `edu_spimview_glue.cpp`, `state.cpp` |
| 123 | **FF — 배치는 두 가지뿐**: ① 에디터 \| (Text·Data 탭) — 시작 배치, ② 에디터·Text·Data를 **한 탭뷰에**. 좌우를 뒤집는 Mirrored는 **삭제**했다 — 가르치는 것이 없고 설명할 것만 하나 늘었다. 액션 이름도 `action_Edu_LayoutSplit` / `action_Edu_LayoutTabbed`로 정리했고(옛 `LayoutPrimary`/`LayoutMirrored`), 프리셋 번호는 0(분할)·1(탭)이다. 크기는 드래그로만 바꾼다 — 비율을 되돌리는 메뉴는 두지 않는다(화면 전체를 되돌리는 Window > Reset Layout은 AA 항목이라 그대로) | 세 가지(Primary/Mirrored/탭) | 검사: `check-editor.sh` 18절이 두 배치의 폭을 960×1080에서 재고, 15·20·24절이 두 배치 모두에서 돈다 | 1.2.3 · `edu_spimview_glue.cpp`, `edu_editor_glue.cpp` |
| 124 | **DD — 패널 머리는 한 가지 모양**: 1.2.2까지 한 화면에 세 가지가 섞여 있었다 — ⓐ 탭만(레지스터, Text·Data), ⓑ 제목 줄만(Editor, Instruction Inspector), ⓒ 제목 줄 **위에** 또 탭("Console / Messages" 도크 안의 두 탭). Y에서 띄우기를 없앤 뒤 ⓑ의 제목 줄에는 이름과 ✕밖에 남지 않았다. 이제 **모든 패널이 탭 띠 하나**를 쓴다: Qt가 다른 패널과 탭으로 묶은 도크는 Qt의 탭 바를, 혼자인 도크는 `EduPanelStrip`(탭 하나짜리 `QTabBar`)을 제목 줄 위젯으로 쓴다. 둘 다 `QTabBar`라서 글꼴·높이·여백이 QSS 한 곳(`QTabBar::tab`)에서 나온다. 하단 패널은 자기 두 탭이 곧 띠이므로 도크 제목을 없앴다. ✕는 **띠 오른쪽 끝 한 자리**다 — 우리 띠는 레이아웃의 마지막 위젯, 하단 패널은 `QTabWidget`의 코너 위젯, Qt의 탭 바는 코너가 없으므로 탭 바의 자식으로 두고 `eduPlaceTabClose()`가 오른쪽 끝에 놓는다(탭이 없는 잔여 바에는 숨긴다) | 세 가지 모양 | **왜 이 방법인가**: 도크를 통째로 우리 `QTabWidget`에 넣으면 Window 메뉴·튜토리얼·`tabifyDockWidget` 기반의 배치(FF)가 모두 무너진다. Qt의 탭 바는 그대로 두고 모자란 곳만 같은 모양으로 채우는 쪽이 FF와 충돌하지 않는다. 덤: 탭 글자 굵기를 선택 여부와 무관하게 고정했다 — `QTabBar`는 평상시 글꼴로 탭 폭을 계산해서, 선택되면 굵어지는 탭은 제 폭을 넘어 첫 글자와 끝 글자가 잘렸다. 긴 이름("Instruction Inspector")을 위해 `StripTabBar::tabSizeHint()`가 글자 폭 + 여백을 보장한다. 검사: `check-editor.sh` 23절 | 1.2.3 · `edu/edu_panel_strip.*`, `edu_spimview_glue.cpp`, `theme/light.qss` |
| 125 | **EE — 패널은 없어질 만큼 눌리지 않는다**: 모든 도크에 최소 크기(`kPanelMinWidth` 200 × `kPanelMinHeight` 104 — 탭 띠와 세 줄쯤)를 주고, 교차 핸들이 쓰는 `eduSetSplitSizes()`도 그 값으로 경계를 잡는다(전에는 120×80). 창 자체의 최소 크기는 820×560 — 그보다 작으면 여섯 패널이 최소치를 지킬 수 없다 | 최소가 0이라 경계를 끝까지 끌면 아래 두 패널이 사라졌다 | 재현: `--squeeze-report`가 교차점을 네 모서리로 끌고 여섯 패널의 크기를 잰다. 최소치를 끄면 1280×720에서 `intregs 187x584 TOO SMALL`, 켜면 두 창 크기 모두에서 통과. 검사: `check-editor.sh` 24절 | 1.2.3 · `theme/tokens.h`, `edu_editor_glue.cpp`, `edu_spimview_glue.cpp`, `spimview.cpp` |
| 126 | **CC — 튜토리얼 마지막 카드가 실제 동작을 말한다**: "예제는 열어 둘 테니"는 1.2.0에서 예제를 닫게 되면서 틀린 말이 되었다. 이제 "끝내면 예제는 닫히고 시작 화면으로 돌아간다 — 사라진 것이 아니라 여러분 차례"라고 말한다. 좌우 뒤집기(FF에서 삭제)와 Window > Tile 언급도 두 배치와 Reset Layout로 고쳤다 | 옛 동작을 설명했다 | 예제가 사라진 것을 오류로 읽지 않게 하는 것이 요점이다. 카드는 1366×768·1920×1080에서 여전히 창 안에 들어온다(`--tutorial-report`) | 1.2.3 · `edu_tutorial.cpp` |
