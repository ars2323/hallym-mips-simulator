...
| 63 | 화면에 남은 원본 이름: 코어가 내는 어셈블 에러 `spim: (parser) … on line N of file …`(로그 창). 에러 목록은 이 접두사를 떼고 보여 준다 | 같음 | 코어 출력이라 `CPU/` 수정 없이는 바꿀 수 없고, `edu/core/edu_asm_errors.cpp`의 파서와 로그 골든이 이 형식을 전제한다 | H2 · 미변경(기록용) |
