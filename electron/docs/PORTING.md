...
**툴바.** 좁아지면 `app.ts fitTitlebar()` 가 한 단계씩 양보한다: 단축키 표시 → 버튼의 아이콘 → 속도를 버튼 하나로
("Speed: Instant") → 간격 좁히기 → 파일명 줄이기 → 맨 마지막에 프로그램 이름(로고는 남음).
파일명은 줄기의 뒤를 자르고 확장자는 남긴다(`hw03_2021….s`, `logic/names.ts`, 한글은 두 칸으로 센다). 전체 이름은 도구 설명에 둔다.
최소 10칸까지 줄여도 모자랄 때만 프로그램 이름을 버리고, 그 자리를 파일명이 다시 쓴다. 파일명은 길어도 32칸까지만 보인다.
`lab04_김학현_20210123.s`(20칸, 한글 포함)로도 1280·1093·1024·910 에서 "Hallym MIPS" 가 남는다. Windows 의 창 버튼 자리
(Linux 보다 약 30px 넓다)를 흉내 낸 `--caption-extra: 30px` 로도 확인한다(e2e). 910 + Windows 에서는 파일명이
`lab04_김….s` 까지 줄어든다. 최소 창(760)에서는 이름이 빠진다. 버튼의 이름(Assemble·Run·Step·Reset)은 끝까지 남는다. 속도에는 "Run speed" 라는 이름을
...
**가리키기.** 대상은 파란 테두리로 두르고 나머지는 옅게(`rgba(0,32,91,.26)`, 글과 패널이 읽히는 선) 덮는다.
붙어 있는 대상(9단계의 비트 칸)은 테두리를 좁혀 칸마다 따로 두르고, 한 상자로 붙지 않게 한다. 덮인 곳은 눌리지 않는다.
...
Data 워드, 18단계의 `syscall` 줄과 Console, 4단계의 Editor 17행과 Text 두 행)은 Run 쪽 대상만 가리킨다. 문장도 그에 맞춰 바뀐다.
14단계는 거터 칸과 그 줄을 한 테두리로 묶는다(한 동작). 20단계는 `tutorial.s` 로 돌아가 어셈블한 온전한 화면에서 끝난다
(19단계의 오류 파일 위가 아니다).
