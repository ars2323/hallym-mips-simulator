...
zip과 MSI는 릴리스 v1.0.1의 자산을 쓴다.

## 1. 설치와 실행

- [ ] `HallymMIPS-1.0.1-win64.zip`을 풀고 `HallymMIPS.exe` 실행. 관리자 권한 없이 뜬다.
- [ ] `HallymMIPS-1.0.1-win64.msi` 설치 → 시작 메뉴에 "Hallym MIPS Simulator".
- [ ] 1.0.0이 설치된 PC에서 1.0.1 MSI가 **덮어쓰기(업그레이드)** 된다. 항목이 두 개 남지 않는다.
...
투어는 **Help > Tutorial**로 다시 연다. 매 단계에서 볼 것은 세 가지다.
...
- [ ] 편집 중인 내용이 있는 상태에서 **Help > Tutorial**을 열어도 "저장할까요?" 창이 뜨지 않고, 편집 중인 글이 그대로 남는다. 1단계 카드가 "지금 열려 있는 프로그램으로 진행합니다"라고 말한다.
