...
const MUTANTS: Mutant[] = [
  ...
  { module: 'window', file: 'src/renderer/app/app.ts', what: 'the Editor keeps 40% however narrow the window',
    find: 'Math.min(760, 0.4 * inner, inner - runLeast)', replace: 'Math.min(760, 0.4 * inner)', tests: ['tests/e2e/fit.e2e.ts'] },
  ...
  { module: 'tutorial', file: 'src/renderer/app/tutorial.ts', what: 'targets left out of view',
    find: '    if (step.reveal && (rects.missing || rects.clipped) &&', replace: '    if (false &&', tests: ['tests/e2e/tutorial.e2e.ts'] },
  ...
  { module: 'tutorial', file: 'src/renderer/app/app.css', what: 'two Harams on screen',
    find: 'body.tutorial-on img.char:not(.tut-char) { visibility: hidden; }', replace: '', tests: ['tests/e2e/tutorial.e2e.ts'] },
