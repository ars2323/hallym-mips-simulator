...
| Assemble | 단축키 **F3** (원본 단축키는 F5·Shift-F5·F10뿐, MARS와 같은 키). **확인 없이 저장**하고 어셈블한다 — 경로 없는 새 파일만 Save As. 성공하면 상태바 "Saved and assembled" | 학생은 Assemble을 계속 누른다. 매번 묻는 것은 방해 (7단계 체크포인트) |
...
- **Assemble (F3)**: 확인 없이 저장(경로 없는 새 파일만 Save As) → 원본 "Reinitialize and Load File" 경로 그대로 호출 → **성공 시 Text 탭으로 전환, 실패 시 Editor에 머물고 에러 줄 표시**
