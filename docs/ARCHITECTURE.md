...
| 66 | **앱 아이콘이 크기마다 다른 공식 마크**: 16px은 심볼 기본형, 24·32·48·64·256px은 원형 엠블럼 A. 마크는 변형하지 않고 축소·여백만. `.ico`는 크기마다 다른 이미지를 담아야 해서 `tools/make-theme-icons.py`가 직접 쓴다(Pillow의 `sizes=`는 한 이미지를 리사이즈할 뿐) | 심볼 한 종류 | 16px에서 엠블럼의 글자 링이 뭉개진다. 근거 캡처 `docs/design/captures/app-icon-options.png` | H2 · `tools/make-theme-icons.py`, `theme/brand/HallymMIPS.ico`, `edu_theme.cpp` `apply` |
