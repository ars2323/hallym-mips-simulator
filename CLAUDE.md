...
5. 단계 종료 시 **사람 체크포인트**: 실제 모니터(Linux)에서 사람이 확인할 체크리스트를 제시하고 멈춘다. 사람 승인 없이 다음 단계로 넘어가지 않는다. Windows zip 확인은 단계마다 하지 않고 **7단계 진입 전과 8단계**에서 한 번에 한다 — 그 사이에는 push마다 도는 CI(Windows 빌드·단위 테스트·패키징)가 초록불이어야 한다.
