...
- 표시 언어: 메뉴·대화상자는 전부 영어다. 한국어 번역(.ts). 최초 실행 투어만 한/영 두 언어로 되어 있다.
- **투어**: 지금은 패널 7개를 짚는 데서 끝난다. "Ctrl+S를 눌러 보세요" 같은 실습형 단계, 진도 저장, 개별 기능(브레이크포인트, Go to, 레이아웃 프리셋)의 짧은 도움말.
- **앱 아이콘**: 24px 이상은 원형 엠블럼, 16px은 심볼이다. 두 마크가 섞이는 것이 어색하면 전부 심볼로 통일하는 선택지가 있다(`docs/design/captures/app-icon-options.png`).
