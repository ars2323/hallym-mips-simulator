...
화면 안으로 스크롤되어 들어옵니다. 자리가 있으면 그 줄 끝에 **Changed** 표가 붙고, 상태 표시줄도 같은 노란색으로
그 이름을 댑니다("방금 바뀜: `$t1`"). QtSpim 도 바뀐 레지스터를 빨간 글씨로 표시하지만, 목록은 번호순이고
진법은 하나입니다.
...
Run 쪽에 Errors 패널이 나옵니다. 맨 위의 제목은 무엇이 잘못됐는지, 그 아래 한 줄은 할 일입니다: "코드에
오류가 있습니다" / "아래 줄을 고친 뒤 Ctrl+S 키를 다시 누르세요." 그 아래에는 오류가 난 줄과 고치는 요령,
**14행으로 가기** 버튼이 있고, 편집기의 그 줄에도 표시가 붙습니다. 그림의 실수처럼 흔한 오타는 요령이
그 오타를 짚습니다: "레지스터 이름 앞에는 $ 기호가 있어야 합니다: t1 → $t1."
...
학생이 오류 문장을 스스로 읽는 연습이 됩니다. 흔한 오타는 요령이 그 자리에서 짚고(예: `t1` → `$t1`,
`.asciz` → `.asciiz`), 그 밖의 문법 오류에는 확인할 곳을 적은 한 줄이 붙습니다: "명령 이름, 레지스터
이름(예: `$t0`), 쉼표를 확인해 보세요."
...
그 비트와 Encoding 값, Data 탭, 라벨, `sw`, 스택, 브레이크포인트, Run, 천천히 실행, Reset, Console, 오류,
마무리. 카드마다 짧은 글 하나입니다. 해 보는 단계는 학생이 카드의 말대로 하면(예: F10) 넘어가고, 볼 것이
있는 단계는 먼저 방금 바뀐 곳(레지스터, 메모리의 값, 프로그램의 출력)을 가리킨 뒤 **다음**을 기다립니다.
QtSpim 은 주 창과 Console 창(그림에서 주황 테두리)을 띄우고, 무엇부터 할지는 정해 두지 않습니다.
