...
- **배치 고르기**: Window > Layout에 세 가지가 있습니다.
  - **Editor | Text / Data** — 위 그림 그대로(처음 상태).
  - **Text / Data | Editor** — 좌우를 바꾸고 아래도 함께 바꿉니다(가운데 아래가 Inspector, 오른쪽 아래가 Console / Messages).
  - **Editor / Text / Data in one place** — 셋을 **한자리에 탭으로** 모읍니다. 창을 화면의 좌우 절반(1920 화면이면 960px)으로 띄울 때 쓰세요. 두 열로 나누면 각 열이 300px밖에 안 되지만, 이 배치에서는 패널 하나가 창 폭을 거의 다 씁니다. 탭 뒤에 있는 패널이 "Source changed" 상태가 되면 그 탭에 점(●)이 붙습니다.
