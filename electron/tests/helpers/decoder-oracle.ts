/* src/core/decoder.ts against the SPIM core, through the addon (the
   spike's check 2; tests/spike/check-decoder.ts and
   tests/core/decoder.test.ts both run it).

   Every instruction the core
...
export interface DecoderReport {
  ...
  quirks: Map<string, number>;
  release2: Map<string, number>;
  failures: string[];
  assemblerErrors: number;
}

export function checkDecoder(program: string): DecoderReport {
  const result = spim.assemble(readFileSync(program));
  ...
  return {
    ...
    distinctWords: new Set(text.map((t) => t.word)).size,
    mnemonics: names.size,
    fieldChecks, formats, quirks, release2, failures,
    assemblerErrors: result.ok ? 0 : result.errors.length,
  };
}
